...
```bash
uv run orcaslicer-profile-explorer && dot -Tpdf orcaslicer_graph.dot -o orcaslicer_graph.pdf
```
...
The `--show-profile` option generates a table of settings show where each setting is set or overriden in the profile inheritance chain. The output is a markdown table that is best view by passing ot a markdown viewer or formatter, for example using [`glow`](https://github.com/charmbracelet/glow)  
...
Compare settings across the inheritance chain and format the markddown output using `glow`:

```bash
uv run orcaslicer-profile-explorer --show-profile "Spool Fuel Generic PETG" | glow -w 120
```

This outputs a table showing how settings are defined across the inheritance chain, from base to specific profile.

Show effective settings for a profile.  The effective settings are generated by combining all settings for the selected profile thorugh the inheritance chain and listing the full set in a single collumn, using inherited values where not set directly.
...
The tool expects OrcaSlicer profiles in the standard location for the operating system:
